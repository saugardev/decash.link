  ...
  return (
    <>
      ...
      <div className="border-text-foreground relative mx-auto my-10 w-full max-w-md overflow-x-auto border sm:rounded-[15px]">
        {isLoading ? (
          <div role="status" className="flex flex-col gap-5 justify-center items-center h-64">
              ...
              {Array.isArray(transactions) && transactions.map((tx, index) => (
                <tr key={index} className="border-text-foreground hover:bg-accent border-b transition-all duration-200">
                  ...
                  <td className="px-6 py-4 text-right">
                      <button onClick={() => handleClaim(tx)} className="font-medium hover:underline">Claim</button>
                  </td>
                </tr>
              ))}
            </tbody>
