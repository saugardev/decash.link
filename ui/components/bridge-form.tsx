      ...
      <div className="flex size-[400px] flex-col justify-between rounded-2xl border bg-white">
        ...
        <div className="mt-5 flex h-16 items-center border-t text-xs relative">
          <div className="mx-5 flex w-full items-center justify-between">
            ...
            <Button variant="ghost" size="icon" onClick={() => setShowChainOptions(!showChainOptions)}>
              {showChainOptions ? <ChevronUpIcon className="size-4" /> : <ChevronDownIcon className="size-4" />}
            </Button>
          </div>
          {showChainOptions && (
            <div className="absolute top-16 left-0 right-0 z-10 flex flex-col bg-white border rounded-[15px] shadow-lg mt-2">
              <button className="px-4 py-2 hover:bg-accent rounded-t-[15px]" onClick={() => handleChainSelect("Sepolia")}>Sepolia</button>
              <button className="px-4 py-2 hover:bg-accent" onClick={() => handleChainSelect("Polygon zkEVM Cardona")}>Polygon zkEVM Cardona</button>
              <button className="px-4 py-2 hover:bg-accent rounded-b-[15px]" onClick={() => handleChainSelect("Astar zKyoto")}>Astar zKyoto</button>
            </div>
          )}
        </div>
      </div>
