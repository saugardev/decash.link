...
import { ChevronDownIcon, ChevronRightIcon, PiggyBank, PiggyBankIcon, XIcon } from "lucide-react";
...
export default function ClaimForm({ claimId: initialClaimId }: { claimId: string | undefined }) {
  ...
  const { data: hash, writeContractAsync: claimPaymentLink } = useWriteContract();
  ...
  return (
    <section className="mx-auto flex flex-col items-center">
      ...
      {claimId && (
        <Button size={"lg"} className="mt-5 flex items-center gap-2 self-end" onClick={handleClaim}>
          Claim
          <ChevronRightIcon className="size-4" />
        </Button>
      )}
